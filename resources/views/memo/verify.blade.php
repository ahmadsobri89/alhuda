...
<title>Pengesahan Memo · {{ $memo->memo_number }}</title>
<style>
...
.urgency-normal       { background: rgba(255,255,255,.2); color: #fff; }
.urgency-urgent       { background: #fef3c7; color: #92400e; }
.urgency-confidential { background: #111827; color: #fff; }
...
<div class="card">
    <div class="card__header">
        ...
        <div class="card__ref-num">{{ $memo->memo_number }}</div>
    ...
    <div class="card__body">
        ...
        <div class="info-grid">
            <div>
                ...
                <div class="info-group__val">{{ $memo->patient->name }}</div>
            </div>
            <div>
                <div class="info-group__lbl">No. Kad Pengenalan</div>
                <div class="info-group__val">{{ $memo->patient->ic_number }}</div>
            </div>
            <div>
                <div class="info-group__lbl">Tarikh Memo</div>
                <div class="info-group__val">{{ $memo->issue_date->translatedFormat('d F Y') }}</div>
            </div>
            <div>
                <div class="info-group__lbl">Dikeluarkan Oleh</div>
                <div class="info-group__val">{{ $memo->issued_by }}</div>
            </div>
        </div>
        ...
        <div class="referred-box">
            ...
            <div>
                <div class="referred-box__lbl">Ditujukan Kepada / Addressed To</div>
                <div class="referred-box__val">{{ $memo->addressed_to }}</div>
                <div class="referred-box__dept">Perkara: {{ $memo->subject }}</div>
            </div>
        </div>

        <div class="reason-box">
            <div class="reason-box__lbl">Isi Memo / Memo Content</div>
            {{ $memo->content }}
        </div>

        <div class="issued-row">
            <div>Dikeluarkan oleh: <span>{{ $memo->issued_by }}</span></div>
            <div>{{ $memo->created_at->format('d/m/Y H:i') }}</div>
        </div>
...
<div class="page-footer">
    Disahkan: {{ now()->format('d/m/Y H:i') }} · {{ $memo->memo_number }}
</div>
